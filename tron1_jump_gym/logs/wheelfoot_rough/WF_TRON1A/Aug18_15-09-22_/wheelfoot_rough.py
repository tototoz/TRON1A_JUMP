...
class BipedWF_Rough(BaseTask):
    ...
    def _reward_torques(self):
        ...
        torques[self.commands[:, 3]  == 1.0, 2] = 0.0
        torques[self.commands[:, 3]  == 1.0, 6] = 0.0
    ...
    def _reward_action_rate(self):
        ...
        scale[self.commands[:, 3]  == 1.0, 2] = 0.0
        scale[self.commands[:, 3]  == 1.0, 6] = 0.0
    ...
    def _reward_base_height(self):
        ...
        base_height[(self.commands[:, 4] == 1)] += 0.16
        base_height[(self.commands[:, 3] == 1)] -= 0.35
    ...
    def _reward_jump_up(self):
        ...
        base_height = torch.mean(self.root_states[:, 2].unsqueeze(1) - self.measured_heights, dim=1)
        root_h_error_jump = torch.sqrt(torch.square(base_height- 1.1 ))
        ...
        foot_error = torch.abs(foot_height  - base_height - 0.3) 
        foot_error_rwd_jump =  5 * torch.exp(-torch.abs(foot_error) * 10)
    ...
    def _reward_feet_clearance_cmd_linear(self):
        ...
        target_height = self.gaits[:, 3].unsqueeze(1) * phases + 0.0453 # offset for foot radius 2cm
