...
class WheelfootController:
    ...
    def compute_observation(self):
        ...
        command_scaler = np.diag([
            ...
            self.user_cmd_cfg['jump'],  # Scale factor for jump 
            self.user_cmd_cfg['sit']  # Scale factor for sit
        ])
    ...
    def sensor_joy_callback(self, sensor_joy: datatypes.SensorJoy):
        ...
        if self.start_controller and sensor_joy.buttons[4] == 1 and sensor_joy.buttons[12] == 1:
            jump_cmd = 1
        else:
            jump_cmd = 0

        if self.start_controller and sensor_joy.buttons[4] == 1 and sensor_joy.buttons[13] == 1:
            sit_cmd = 1
        else:
            sit_cmd = 0
        ...
        self.commands[3] = jump_cmd 
        self.commands[4] = sit_cmd 
